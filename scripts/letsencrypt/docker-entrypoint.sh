...
if [ "$LETSENCRYPT_MODE" == "staging" ]; then
    ...
    certbot --config-dir "/calculadora-certificates/$LETSENCRYPT_MODE" certonly --webroot -w "/calculadora-certificates" -d "$HTTPS_HOST" -m "$ADMIN_EMAIL" --agree-tos --non-interactive --test-cert
elif [ "$LETSENCRYPT_MODE" == "production" ]; then
    printf "\nTrying to get PRODUCTION certificate\n"
    certbot --config-dir "/calculadora-certificates/$LETSENCRYPT_MODE" certonly --webroot -w "/calculadora-certificates" -d "$HTTPS_HOST" -m "$ADMIN_EMAIL" --agree-tos --non-interactive --server https://acme-v02.api.letsencrypt.org/directory
